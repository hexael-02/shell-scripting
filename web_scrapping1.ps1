# Configuracion de consola para evitar errores de salida
...
function Start-SuperScraper {
    ...
    try {
        # 1. Definir rutas (Se creara en la misma carpeta del script)
        ...
        "=== REPORTE PARA: $Url ===`r`n`nTITULOS:`r`n$($titles -join "`r`n")`r`n`nPRECIOS:`r`n$($prices -join "`r`n")`r`n`nCORREOS:`r`n$($emails -join "`r`n")" | Out-File $reportTxt -Encoding UTF8
        ...
        $imgUrls = [regex]::Matches($content, '(?<=<img [^>]*src=")(https?://[^"]+\.(jpg|png|webp))') | Select-Object -ExpandProperty Value -Unique | Select-Object -First 5
